        ...
        if user:
            if check_password_hash(user.password, password):
                flash("Logged in successfully!",
                      category='success')  # Flash message
                # remember user if user logs in
                login_user(user, remember=True)
                # redirect back to dashboard page when user logs in
    ...
    if request.method == 'POST':  # Post. Update these details to the database
        ...
        if user:
            ...
            new_user = User(email=email, username=username, password=generate_password_hash(
                password1, method='scrypt:32768:8:1'))  # generate a hashed password. Encrypt the password
            db.session.add(new_user)  # add something to database
            db.session.commit()  # commit to database
            # if new user create a new user, and remember
